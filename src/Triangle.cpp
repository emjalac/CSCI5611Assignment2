...
Vec3D Triangle::calculateVelocity(Vec3D wind)
{
	Vec3D vel = (1/3 * (v1->getVel()+v2->getVel()+v3->getVel())) - wind;
	return vel;
}

Vec3D Triangle::calculateDrag(Vec3D wind)
{
	//using optimized formula from class slides on cloth simulation
	Vec3D vel = calculateVelocity(wind);
